...
Hmm newHmm(int statesCount, int symbolsCount) {
    Hmm model = {.statesCount = statesCount, .symbolsCount = symbolsCount};
    model.PI = (double *) malloc(statesCount * sizeof(double));
    model.PHI = (double *) malloc(statesCount * sizeof(double));
    model.A = (double **) malloc(statesCount * sizeof(double *));
    model.B = (double **) malloc(statesCount * sizeof(double *));
    int i;
    for (i = 0; i < statesCount; i++) {
        model.A[i] = (double *) malloc(statesCount * sizeof(double));
        model.B[i] = (double *) malloc(symbolsCount * sizeof(double));
    }
    return model;
}
...
void freeHmm(Hmm model) {
    ...
    free(model.A);
    free(model.B);
    model.statesCount = model.symbolsCount = 0;
}
...
SequencesSet newSequencesSet(int count, int lengths[]) {
    ...
    SequencesSet result;
    result.count = count;
    result.sequence = (Sequence *) malloc(count * sizeof(Sequence));
    for (i = 0; i < count; ++i) {
        result.sequence[i] = newSequence(lengths[i]);
    }
    return result;
}
...
void freeSequencesSet(SequencesSet sequencesSet) {
    int i;
    for (i = 0; i < sequencesSet.count; ++i) {
        freeSequence(sequencesSet.sequence[i]);
    }
    free(sequencesSet.sequence);
    sequencesSet.count = 0;
}
